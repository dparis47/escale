...
export function SectionCVLMEI({ accompagnementId, cvs: cvsInit }: Props) {
  ...
  async function telechargerCv(cid: number, nom: string) {
    const res = await fetch(`/api/accompagnement/${accompagnementId}/cvs-ei/${cid}`)
    if (!res.ok) return
  ...
  async function uploaderCv(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch(`/api/accompagnement/${accompagnementId}/cvs-ei`, { method: 'POST', body: form })
    setCvEnCours(false)
    if (!res.ok) {
  ...
  async function supprimerCv(cid: number, nom: string) {
    ...
    setCvEnCours(true)
    await fetch(`/api/accompagnement/${accompagnementId}/cvs-ei/${cid}`, { method: 'DELETE' })
    setCvEnCours(false)
